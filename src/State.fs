...
module ActivePiece =
    let init tetromino =
        { Tetromino = tetromino
          Position = { X = Board.width / 2 - 1; Y = 2 }
          Rotation = Up }
...
let nextPiece eligiblePieces queued =
    let (nextTetromino, queued) =
        ...
        | [] ->
            let xs =
                [ for _ in 0 .. 5 -> randTetromino eligiblePieces ] in (List.head xs, List.tail xs)
...
let initGameState =
    ...
    { Paused = false
      ...
      Clock = { Ticks = 0L; DropFrequency = 75L } }
...
let bindKeys (dispatch: Dispatch<Msg>) =
    Browser.Dom.document.addEventListener (
        "keydown",
        (fun (evt: Browser.Types.Event) ->
            Browser.Dom.console.log ("KEYDOWN")

            let msg =
                match ((evt :?> Browser.Types.KeyboardEvent).code) with
                | "KeyP" -> TogglePaused |> Some
                | "Escape" -> TogglePaused |> Some
                | "KeyR" -> TriggerRestart |> Some

                | "KeyL" -> LambdaMode |> Some

                | "Space" -> HardDrop |> UpdateActivePiece |> Some // Spacebar
                | "KeyC" -> Hold |> UpdateActivePiece |> Some
                | "ArrowUp" ->
                    UpdateRotation Clockwise
                    |> UpdateActivePiece
                    |> Some
                | "KeyZ" ->
                    UpdateRotation CounterClockwise
                    |> UpdateActivePiece
                    |> Some
                | "ArrowRight" ->
                    ...
                    |> UpdateActivePiece
                    |> Some
                | "ArrowDown" -> Drop |> UpdateActivePiece |> Some
                | "ArrowLeft" ->
                    OffsetPosition { X = -1; Y = 0 }
                    |> UpdateActivePiece
                    |> Some
...
let handleTick (model: GameState) : GameState * Cmd<Msg> =
    ...
    let subs =
        [ (fun dispatch ->
              (fun () -> dispatch Tick)
              |> FPWindow.setTimeout (float tickFrequency * 0.1<ms>)) ]
    ...
    | false ->
        ...
        let model' =
            { model with Clock = { model.Clock with Ticks = ticks' } }

        //Browser.Dom.console.log ($"Tick: {ticks'} - DropFreq: {dropFreq} ")
        ...
        model',
        match dropSubOpt with
        | Some dropSub -> dropSub :: subs
        | None -> subs

let rec droppedPlacement board piece =
    let pos' =
        let { X = x; Y = y } = piece.Position in { X = x; Y = y + 1 }

    let piece' = { piece with Position = pos' }

    if piece' |> validatePiece board then
        piece' |> droppedPlacement board
    else
        piece
...
let clearLines (board: Board) =
    ...
    let (_, _, clearedBoard) =
        ...
        |> Seq.fold
            (fun (downOffset, (fullRowsDesc: int list), board) (y, cells) ->
                match y with
                | _ when not fullRowsDesc.IsEmpty && y = fullRowsDesc.Head ->
                ...
                | _ ->
                    let board' =
                        cells
                        |> Seq.fold
                            (fun acc ({ X = x; Y = y }, cell) -> acc |> Map.add { X = x; Y = y + downOffset } cell)
                            board
    ...
    let board' =
        ...
        |> Seq.fold
            (fun acc y ->
                seq { 0 .. Board.width - 1 }
                |> Seq.fold (fun acc x -> acc |> Map.add { X = x; Y = y } None) acc)
            clearedBoard

    board'

let update msg model : GameState * Cmd<Msg> =
    match msg with
    | Tick -> handleTick model
    | TogglePaused -> { model with Paused = not model.Paused }, []
    | TriggerRestart -> initGameState, []
    | LambdaMode -> { model with EligiblePieces = model.EligiblePieces |> Set.add Λ }, []
    | UpdateActivePiece apMsg when not model.Paused ->
        match apMsg with
        | Drop ->
            if model.ActivePiece
               |> validatePiece model.PlacedBoard
               |> not then
                // game over
                model, []
            else
                let activePiece' =
                    let pos' =
                        let { X = x; Y = y } = model.ActivePiece.Position in { X = x; Y = y + 1 }

                    { model.ActivePiece with Position = pos' }

                let isValid =
                    activePiece' |> validatePiece model.PlacedBoard
                ...
                let model' =
                    ...
                    | false ->
                        ...
                        let activePiece', queued' =
                            nextPiece model.EligiblePieces model.QueuedPieces
        ...
        | HardDrop ->
            let model' =
                let droppedPiece =
                    model.ActivePiece
                    |> droppedPlacement model.PlacedBoard
        ...
        | UpdatePosition pos ->
            ...
            let isValid =
                piece' |> validatePiece model.PlacedBoard

            let model' =
                if isValid then
                    { model with ActivePiece = piece' }
                else
                    model
        ...
        | OffsetPosition offset ->
            model,
            UpdatePosition(model.ActivePiece.Position + offset)
            |> UpdateActivePiece
            |> Cmd.ofMsg
        | UpdateRotation spin ->
            let nextRot =
                model.ActivePiece.Rotation |> Spin.nextRot spin

            let piece' =
                { model.ActivePiece with Rotation = nextRot }
